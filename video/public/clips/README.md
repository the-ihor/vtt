...
Drop your screen-recordings here, then set their paths in `CLIPS` at the top of
`src/HeroVideo.tsx`. Until a path is set, that beat renders a labelled
placeholder, so the whole film previews and exports right now.

| key in CLIPS | suggested file | what to record |
|---|---|---|
...
| `history`   | `07-history.mp4`   | History tab scrolling, Copy buttons |

Record at 1920×1080 (or higher) so clips fill the 16:9 frame cleanly.
