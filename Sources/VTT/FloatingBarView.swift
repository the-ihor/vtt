...
struct FloatingBarView: View {
    ...
    var body: some View {
        pill
            ...
            .padding(20)
            .animation(.spring(response: 0.35, dampingFraction: 0.85), value: state.mode)
            .animation(.spring(response: 0.35, dampingFraction: 0.85), value: state.preparing)
    ...
    private var modeContent: some View {
        ...
        case .idle, .recording:
            HStack(spacing: 7) {
                Waveform(level: state.mode == .recording ? state.level : 0)
                    .frame(width: 64)
                if let liveText {
                    Text(liveText)
                        .font(.system(size: 11))
...
/// shimmer for motion so silence still looks alive.
